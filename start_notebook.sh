...
finish()
{
    if [ "$_arg_gpu" = on ];
	then
	    CURRENT_UID=$(id -u):$(id -g)  docker-compose -f docker-compose.yml -f docker-compose.gpu.yml stop notebook postgres
	else
	    CURRENT_UID=$(id -u):$(id -g)  docker-compose -f docker-compose.yml stop notebook postgres
	fi 
    exit
}
trap finish SIGINT
...
if [ "$_arg_gpu" = on ];
then
    CURRENT_UID=$(id -u):$(id -g) docker-compose -f docker-compose.yml -f docker-compose.gpu.yml up --build -d notebook postgres
else
    CURRENT_UID=$(id -u):$(id -g) docker-compose up --build -d notebook postgres
fi
